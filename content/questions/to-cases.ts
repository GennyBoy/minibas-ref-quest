...
export const toCasesQuestions: Question[] = [
  ...
  },
  {
    ...
    type: 'single',
    prompt:
      '3P採用大会。スリーポイントライン付近のショットが決まったが、審判のシグナルを確認できなかった。どうする？',
    choices: [
      '自分でラインを見て判断して記録する',
      'とりあえず2点または3点で記録し、状況（残り時間・番号・何点目か）をメモして次のデッドで審判に確認する',
      '記録を保留してプレーを見続ける',
    ],
    answer: 1,
    explanation:
      '2点か3点かの判断は審判のシグナルで行うもので、スコアラーがラインを見て判断しない。確認できなかったら仮で記録してメモを残し、次のボールデッドで審判を呼んで確認する。',
    refs: ['JBA TOマニュアル', 'knowledge/10-to-difficult-cases'],
    ruleYear: 2026,
  },
  {
    ...
    difficulty: 2,
    type: 'single',
    prompt: 'ベンチから「うちの4番のファウルは2つのはずだ」と疑義の申し出があった。どう対応する？',
    choices: [
      'その場でスコアシートを見せて説明する',
      'ゲーム再開前ならすぐ審判を呼んで確認。再開後なら「後で確認します」と伝えて次のデッドで確認する',
      '取り合わない',
    ],
    answer: 1,
    explanation:
      'ゲームの進行を止めないのが原則。再開前なら審判を呼んですぐ確認し、再開後は次のデッドで確認する。なおHC・Aコーチのスタッツ確認はボールデッドでクロックが止まっているときのみ認められる。',
    refs: ['TO主任マニュアル 7-(3)', 'knowledge/10-to-difficult-cases'],
    ruleYear: 2026,
  },
  {
    id: 'cs-015',
    ...
    type: 'single',
    prompt:
      'ショット不成功でボールがリングに触れたのに、ショットクロックのリセット操作が漏れて秒数が進み続けている。周りが気づいたら？',
    choices: [
      '次のデッドまで待ってから審判に報告する',
      '気づいた者が「当たった！」「リセット！」とコールして即座にリセット操作を促す',
      'ブザーを鳴らしてゲームを止める',
    ],
    answer: 1,
    explanation:
      'リセット漏れは早く直せばゲームを止めずに済むので、気づいた者（タイマー・TO主任含む）が即コールして操作を促す。リングに触れた瞬間の「当たった／入った」コールの習慣化が予防策。',
    refs: ['TO主任マニュアル 7-(11)', 'knowledge/10-to-difficult-cases'],
    ruleYear: 2026,
  },
  {
    id: 'cs-016',
    ...
    difficulty: 3,
    type: 'single',
    prompt: '審判のファウルレポートで、ファウルを「された」側の番号をコールしている疑いがある。どうする？',
    choices: [
      '疑わしいがレポートどおりに記入する',
      'すぐに審判を呼んで確認する。曖昧なまま記入しない',
      '両方の番号に小さく記入しておく',
    ],
    answer: 1,
    explanation:
      '番号が不明瞭・間違っていそうなときはすぐ審判を呼んで確認する。誤ったプレーヤーへのファウル記入は訂正手順（点・方式）が面倒になる。Aスコアラーも番号を見ておき「今の何番？」に即答できるようにしておく。',
    refs: ['TO主任マニュアル 7-(3)', 'knowledge/10-to-difficult-cases'],
    ruleYear: 2026,
  },
  {
    ...
    difficulty: 2,
    type: 'single',
    prompt: 'ゲーム中に記入ミスを見つけたが、プレーが続いていてすぐには直せない。どうする？',
    choices: [
      'プレー中でも急いで直す',
      '付箋を貼っておき、タイムアウトやインターバルで訂正する',
      'ゲーム終了後にまとめて直す',
    ],
    answer: 1,
    explanation:
      'すぐ直せないときは付箋を貼って目印にし、タイムアウト・インターバルで落ち着いて訂正する。記録が必要な場面を逃さないことが最優先。訂正期限はクルーチーフのサイン前まで。',
    refs: ['JBA TOマニュアル 4.5.2', 'knowledge/10-to-difficult-cases'],
    ruleYear: 2026,
  },
]
